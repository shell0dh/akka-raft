...
private[protocol] trait StateMetadata extends Serializable {
  ...
  sealed trait Metadata {
     ...
     */
    implicit def clusterSelf: ActorRef

    // todo rethink if needed, better yet rewrite ClusterRaftActor to be properly transparent for RaftActor if possible.
    ...
    def membersExceptSelf = config.members filterNot {
      _ == clusterSelf
    }
